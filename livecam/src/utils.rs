...
pub fn load<T: DeserializeOwned>(
    ...
    let mut builder = ConfigRs::builder()
        .set_default("http.listen", "0.0.0.0:9999").unwrap()
        .set_default("log.level", "info").unwrap()
        .set_default("auth.username", "admin").unwrap()
        .set_default("auth.password_hash", "$argon2id$v=19$m=19456,t=2,p=1$bmljZXRyeQ$PqTT/n9ToBNVsdsoquTz1A/P5s9O4yvA9fym5Vd5s9s").unwrap()
        .set_default("cameras", Vec::<String>::new()).unwrap();

        if current_dir_path.exists() {
            builder = builder.add_source(File::from(current_dir_path.clone()).required(false));
            tracing::info!("Loaded config from current directory: {}", current_dir_path.display());
        } else {
            tracing::warn!("Config not found in current directory at {}, checking other sources.", current_dir_path.display());
        }
    ...
    if etc_path.exists() {
        ...
        tracing::warn!("Base config not found at {}, using internal defaults.", etc_path.display());
    }
...
pub fn reset_config(name: &str) -> anyhow::Result<()> {
    ...
    if path.exists() {
        ...
        tracing::info!("No user configuration found at {}, nothing to reset.", path.display());
    }
    Ok(())
}
