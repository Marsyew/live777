...
async fn login_handler(
    ...
    println!("Received username: {}, password: {}", payload.username, payload.password);
    ...
    // // --- 主要修改点：将哈希验证替换为明文密码比较 ---
    
    // // 1. 检查用户名是否匹配
    // let is_username_valid = payload.username == config.auth.username;
    
    // // 2. 直接比较前端传来的密码和配置文件中的明文密码
    // //    (这依赖于您已在 Config 结构体中将 password_hash 改为 password)
    // let is_password_valid = payload.password == config.auth.password_hash;

    // if is_username_valid && is_password_valid {
    //     // 登录成功，生成 JWT Token 的逻辑保持不变
    //     let now = chrono::Utc::now();
    //     let exp = (now + chrono::Duration::hours(24)).timestamp() as usize;
    //     let claims = Claims {
    //         sub: payload.username.clone(),
...
async fn change_password_handler(
    ...
    let new_password_hash = match Argon2::default().hash_password(payload.new_password.as_bytes(), &salt) {
...
async fn reset_config_handler(
    _claims: Claims,
) -> impl IntoResponse {
    if let Err(e) = super::utils::reset_config("livecam") {
         return (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to reset config: {}", e)).into_response();
    }
    (StatusCode::OK, "Configuration reset. Please restart the application to apply changes.").into_response()
}
