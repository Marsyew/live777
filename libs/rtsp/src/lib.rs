...
pub struct Handler {
    ...
    // pub interleaved_tx: Option<UnboundedSender<(u8, Vec<u8>)>>,
    pub interleaved_tx: Option<Arc<tokio::sync::Mutex<UnboundedSender<(u8, Vec<u8>)>>>>,
    // pub interleaved_rx: Option<UnboundedReceiver<(u8, Vec<u8>)>>,
    pub interleaved_rx: Option<Arc<tokio::sync::Mutex<UnboundedReceiver<(u8, Vec<u8>)>>>>,
}
        ...
        match reader.read(&mut buf).await {
            ...
            Ok(n) => {
                ...
                while accumulated_buf.len() >= 4 && accumulated_buf[0] == b'$' {
                    ...
                    //         if let Err(e) = tx.send((channel, rtp_data)) {
...
// pub fn get_current_tcp_writer() -> Option<Arc<Mutex<Box<dyn AsyncWrite + Send + Unpin>>>> {
//     let current = CURRENT_TCP_WRITER.lock().unwrap();
//     current.clone()
// }

// pub fn set_current_tcp_writer(writer: Option<Arc<Mutex<Box<dyn AsyncWrite + Send + Unpin>>>>) {
//     let mut current = CURRENT_TCP_WRITER.lock().unwrap();
//     *current = writer;
// }
